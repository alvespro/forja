...
export function AgendaPage() {
  ...
  return <div className="flex flex-col gap-5">
    <header className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-bold">Agenda</h1><p className="text-sm text-aco-texto">Seu tempo, em um só lugar.</p></div><Button onClick={() => setSelected('new')}>Novo evento</Button></header>
    <div className="flex flex-wrap items-center gap-2"><div className="flex rounded-xl bg-[#1D1D1D] p-1">{(['Dia', 'Semana', 'Mês'] as View[]).map(v => <Button key={v} variant={view === v ? 'default' : 'ghost'} aria-pressed={view === v} onClick={() => setView(v)}>{v}</Button>)}</div><Button variant="outline" disabled={sync.isPending} onClick={() => sync.mutate()}>{sync.isPending ? 'Sincronizando…' : 'Sync Google'}</Button></div>
    ...
    {!events.isLoading && !events.isError && (view === 'Mês' ? <div className="overflow-x-auto"><div className="grid min-w-[560px] grid-cols-7 gap-px overflow-hidden rounded-2xl bg-border">
      {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map(d => <div key={d} className="bg-black p-3 text-center text-xs text-aco-texto">{d}</div>)}
      {days.map(d => { const key = format(d, 'yyyy-MM-dd'); const items = events_on_day(events.data ?? [], key); return <div key={key} className={`min-h-28 space-y-1 bg-[#1D1D1D] p-1 sm:p-2 ${key === today ? 'ring-1 ring-inset ring-brasa' : ''}`}><button aria-label={`Abrir ${format(d, 'dd/MM/yyyy')}`} onClick={() => { setDay(key); setView('Dia') }} className={`flex size-11 items-center justify-center rounded-full text-sm ${key === today ? 'bg-brasa text-black' : d.getMonth() !== date.getMonth() ? 'text-aco-texto' : ''}`}>{d.getDate()}{items.length > 0 && <span className="ml-1 size-1 rounded-full bg-brasa" />}</button>{items.slice(0, 3).map(pill)}{items.length > 3 && <button className="min-h-8 text-xs text-aco-texto" onClick={() => { setDay(key); setView('Dia') }}>+{items.length - 3} eventos</button>}</div> })}
    </div></div> : <div className="overflow-x-auto rounded-2xl border border-border"><div style={{ minWidth: view === 'Semana' ? 840 : undefined }}>
