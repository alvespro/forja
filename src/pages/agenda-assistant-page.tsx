    ...
    <section className="overflow-hidden rounded-2xl border border-border bg-card/60 shadow-sm">
      ...
      <div className="border-t border-border p-4 sm:p-5">
        ...
        <div className="mt-2.5 grid grid-cols-3 gap-2 sm:flex sm:flex-wrap">
          <Button type="button" size="sm" variant="outline" onClick={() => inputRef.current?.click()} disabled={ask.isPending} className="min-h-11"><Icon name="attach_file" size={16} />Imagem</Button>
          <input ref={inputRef} className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) readImage(file).then(setAttachment).catch((error: Error) => toast.error(error.message)); event.target.value = '' }} />
          <Button type="button" size="sm" variant="outline" onClick={() => cameraRef.current?.click()} disabled={ask.isPending} className="min-h-11"><Icon name="photo_camera" size={16} />Câmera</Button>
          <input ref={cameraRef} className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" capture="environment" onChange={(event) => { const file = event.target.files?.[0]; if (file) readImage(file).then(setAttachment).catch((error: Error) => toast.error(error.message)); event.target.value = '' }} />
          <Button type="button" size="sm" variant={listening ? 'default' : 'outline'} onClick={listening ? undefined : startVoice} disabled={ask.isPending || listening} className="min-h-11"><Icon name="mic" size={16} />{listening ? 'Ouvindo…' : 'Falar'}</Button>
          <span className="col-span-3 self-center text-xs leading-5 text-foreground/75 sm:ml-1">Texto, arquivo, foto pela câmera ou ditado em português.</span>
        </div>
      </div>
    </section>
