...
export function AgendaAssistantPage() {
  ...
  return <main className="mx-auto flex w-full max-w-4xl flex-col gap-5">
    <header className="flex flex-wrap items-start justify-between gap-3"><div><p className="ds-label text-brasa">ASSISTENTE DE AGENDA</p><h1 className="mt-1 text-2xl font-bold">Agende conversando</h1><p className="mt-1 text-sm text-aco-texto">A IA entende o pedido e cria somente a prévia. A confirmação é sempre sua.</p></div><Button asChild variant="outline"><Link to="/agenda">Voltar à agenda</Link></Button></header>
    <section className="overflow-hidden rounded-2xl border border-border bg-card/60 shadow-sm">
      <div className="min-h-[22rem] space-y-3 p-4 sm:p-6" aria-live="polite">
      ...
      <div className="border-t border-border p-4 sm:p-5">
        ...
        <form className="flex gap-2" onSubmit={(event) => { event.preventDefault(); send() }}><Textarea value={text} onChange={(event) => setText(event.target.value)} placeholder="Descreva um compromisso…" rows={3} maxLength={6000} disabled={ask.isPending} /><Button type="submit" disabled={ask.isPending || (!text.trim() && !attachment)}>{ask.isPending ? 'Lendo…' : 'Enviar'} <Icon name="arrow_forward" size={17} /></Button></form>
        <div className="mt-3 flex flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={() => inputRef.current?.click()} disabled={ask.isPending}><Icon name="attach_file" size={16} />Imagem</Button><input ref={inputRef} className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) readImage(file).then(setAttachment).catch((error: Error) => toast.error(error.message)); event.target.value = '' }} /><Button type="button" size="sm" variant={listening ? 'default' : 'outline'} onClick={listening ? undefined : startVoice} disabled={ask.isPending || listening}><Icon name="mic" size={16} />{listening ? 'Ouvindo…' : 'Falar'}</Button><span className="self-center text-xs text-aco-texto">Texto, print/foto ou ditado em português.</span></div>
      </div>
    </section>
